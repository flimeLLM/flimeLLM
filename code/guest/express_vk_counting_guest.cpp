...
// Autogenerated module express_vk_counting_guest
//
...
void count_VkFramebufferCreateInfo(uint32_t featureBits, VkStructureType rootType,
    ...
    *count += 1 * 8;
    *count += sizeof(uint32_t);
    if (((((toCount->flags & VK_FRAMEBUFFER_CREATE_IMAGELESS_BIT) == 0)))) {
...
void count_VkPresentInfoKHR(uint32_t featureBits, VkStructureType rootType,
    ...
    *count += sizeof(VkStructureType);
    ...
    count_extension_struct(featureBits, rootType, toCount->pNext, count);
    *count += sizeof(uint32_t); // waitSemaphoreCount
    if (toCount->waitSemaphoreCount) {
        *count += toCount->waitSemaphoreCount * 8; // pWaitSemaphores
    }
    *count += sizeof(uint32_t); // swapchainCount
    if (toCount->swapchainCount) {
        *count += toCount->swapchainCount * 8; // pSwapchains
        *count += toCount->swapchainCount * sizeof(uint32_t); // pImageIndices
    }
    if (toCount->pResults) {
        *count += toCount->swapchainCount * sizeof(VkResult); // pResults
    }
}
...
void count_extension_struct(uint32_t featureBits, VkStructureType rootType,
    ...
    size_t currExtSize = express_vk_extension_struct_size_with_stream_features(
