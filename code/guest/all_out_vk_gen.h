...
// clang-format off
VKAPI_ATTR VkResult CreateAndroidSurfaceKHR(
    VkInstance instance,
...
VKAPI_ATTR VkResult GetSwapchainImagesKHR_special(
    ...
    uint64_t*          pSwapchainBufferHandles);

VKAPI_ATTR VkResult EnumerateInstanceVersion(uint32_t* pVersion);
...
VKAPI_ATTR void GetPhysicalDeviceExternalBufferPropertiesKHR(VkPhysicalDevice physicalDevice, const VkPhysicalDeviceExternalBufferInfo* pExternalBufferInfo, VkExternalBufferProperties* pExternalBufferProperties);
...
VKAPI_ATTR VkResult BindBufferMemory2KHR(VkDevice device, uint32_t bindInfoCount, const VkBindBufferMemoryInfo* pBindInfos);
...
VKAPI_ATTR VkResult AcquireNextImageKHR_special(VkDevice device, VkSwapchainKHR swapchain, uint64_t timeout, VkSemaphore semaphore, VkFence fence, uint32_t* pImageIndex, uint64_t buffer);

VKAPI_ATTR VkResult QueuePresentKHR(VkQueue queue, const VkPresentInfoKHR* pPresentInfo);
VKAPI_ATTR VkResult QueuePresentKHR_special(VkQueue queue, const VkPresentInfoKHR* pPresentInfo, uint64_t* buffer_handles);
